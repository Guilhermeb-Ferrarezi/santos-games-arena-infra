...
export async function verifySessionToken(
  token: string,
  env: Pick<CheckoutApiEnv, "JWT_SECRET">
...
type TokenPayload = SessionTokenUser & { exp: number };

function parsePayload(encodedPayload: string): TokenPayload | null {
  try {
    const decoded = JSON.parse(base64UrlDecode(encodedPayload)) as Partial<TokenPayload>;
