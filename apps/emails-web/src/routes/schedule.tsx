...
import {
  Calendar, Plus, Loader, CheckCircle, XCircle, Clock, AlertCircle, Trash2, Send,
} from "lucide-react";
import {
  fetchScheduled, createScheduled, cancelScheduled, fetchTemplates,
  type ScheduledEmail, type TemplateMeta,
} from "@/lib/api";
              ...
              <div className="space-y-1">
                {templates.map((t) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between px-4 py-3 bg-white/[0.03] hover:bg-white/[0.06] border border-white/[0.05] rounded-sm text-left transition-colors"
                  >
            ...
            <thead>
              <tr className="border-b border-white/[0.06]">
                {["", "Destinatário", "Template", "Agendado para", "Status", ""].map((h, i) => (
                  <th key={i} className="px-5 py-3 text-left text-[10px] font-bold uppercase tracking-widest text-white/25">
                    {h}
                  </th>
                ))}
              </tr>
